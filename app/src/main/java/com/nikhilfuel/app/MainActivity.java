...
public class MainActivity extends Activity {
    ...
    private boolean backHandledByPopup = false;
    ...
    public class AndroidBridge {
        ...
        @JavascriptInterface
        public void popupHandledBack() {
            backHandledByPopup = true;
        }
    }
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        // Required for JavaScript confirm()/alert() dialogs used by delete actions.
    ...
    private void openWhatsApp(String text) {
        ...
        try { startActivity(Intent.createChooser(intent, "Share report")); }
        catch (ActivityNotFoundException e) { Toast.makeText(this, "WhatsApp is not installed", Toast.LENGTH_SHORT).show(); }
    }
    ...
    private void handleBackPress() {
        ...
        backHandledByPopup = false;

        webView.evaluateJavascript(
            "(function(){" +
            "var p=document.getElementById('historyPopup');" +
            "if(p&&getComputedStyle(p).display!=='none'){if(typeof window.closeHistoryPopup==='function')window.closeHistoryPopup();else p.style.display='none';try{AndroidBridge.popupHandledBack()}catch(e){}return;}" +
            "var m=document.getElementById('segmentHistoryOverlay');" +
            "if(m&&m.classList.contains('show')){if(typeof window.closeSegmentHistory==='function')window.closeSegmentHistory();else m.classList.remove('show');try{AndroidBridge.popupHandledBack()}catch(e){}return;}" +
            "m=document.getElementById('denomHistoryOverlay');" +
            "if(m&&m.classList.contains('show')){if(typeof window.closeDenomHistory==='function')window.closeDenomHistory();else m.classList.remove('show');try{AndroidBridge.popupHandledBack()}catch(e){}return;}" +
            "m=document.getElementById('termsOverlay');" +
            "if(m&&m.classList.contains('show')){if(typeof window.closeTerms==='function')window.closeTerms();else m.classList.remove('show');try{AndroidBridge.popupHandledBack()}catch(e){}return;}" +
            "m=document.getElementById('appInfo');" +
            "if(m&&m.classList.contains('show')){if(typeof window.closeAppInfo==='function')window.closeAppInfo();else m.classList.remove('show');try{AndroidBridge.popupHandledBack()}catch(e){}return;}" +
            "})()", null);

        new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
            @Override public void run() {
                if (backHandledByPopup || webView == null) return;
